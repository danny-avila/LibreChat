...
- **The existing backlog is recorded, not exempted.** `eslint-suppressions.json` holds the
  violations the tree carried when the rules landed (3,121 today), as a per-file, per-rule count.
  ...
  some); `npm run lint:design:suppress` re-records the design rules, which is what a file move
  needs, since suppressions are keyed by path. Neither command changes which rules run; both only
