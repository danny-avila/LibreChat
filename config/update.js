...
(async () => {
  ...
  if (docker) {
    ...
    console.purple('Pruning all Brainiac Docker images...');

    const imageName = singleCompose ? 'brainiac_single' : 'brainiac';
    try {
      execSync(`${sudo}docker rmi ${imageName}:latest`, { stdio: 'inherit' });
    } catch (e) {
      console.purple('Failed to remove Docker image brainiac:latest. It might not exist.');
    }
    console.purple('Removing all unused dangling Docker images...');
    execSync(`${sudo}docker image prune -f`, { stdio: 'inherit' });
    console.purple('Building new Brainiac image...');
  ...
  console.green('Your Brainiac app is now up to date! Start the app with the following command:');
  console.purple(startCommand);
  console.orange(
    "Note: it's also recommended to clear your browser cookies and localStorage for Brainiac to assure a fully clean installation.",
  );
