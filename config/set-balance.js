...
const { getBalanceConfig } = require('@brainiac/api');
const { User, Balance } = require('@brainiac/data-schemas').createModels(mongoose);
...
(async () => {
  ...
  if (!balanceConfig?.enabled) {
    console.red('Error: Balance is not enabled. Use brainiac.yaml to enable it');
    silentExit(1);
  }
