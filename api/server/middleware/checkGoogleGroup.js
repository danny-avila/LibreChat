...
const verifyGoogleGroupMembership = async (req, res, next) => {
  ...
  if (!req.user || !req.authInfo || !req.authInfo.accessToken) {
    logger.error('[verifyGoogleGroupMembership] Missing user or authInfo. Ensure Passport authentication ran correctly.');
  ...
  try {
    ...
    if (!isMember) {
      logger.warn(`[verifyGoogleGroupMembership] User ${userEmail} denied access: not a member of ${GOOGLE_WORKSPACE_GROUP}`);
    ...
    logger.info(`[verifyGoogleGroupMembership] User ${userEmail} verified as member of ${GOOGLE_WORKSPACE_GROUP}`);
    return next();
  } catch (error) {
