...
} = require('@brainiac/data-schemas');
const { ErrorTypes, SystemRoles, errorsToString } = require('brainiac-data-provider');
const { isEnabled, checkEmailConfig, isEmailDomainAllowed, math } = require('@brainiac/api');
const {
  findUser,
  findToken,
  createUser,
...
const sendVerificationEmail = async (user) => {
  ...
  await sendEmail({
    ...
    payload: {
      appName: process.env.APP_TITLE || 'Brainiac',
...
const setAuthTokens = async (userId, res, _session = null) => {
  try {
    ...
    res.cookie('token_provider', 'brainiac', {
      expires: new Date(refreshTokenExpires),
      httpOnly: true,
      secure: isProduction,
      sameSite: 'strict',
