...
const { extractMCPServers, MCPOAuthHandler } = require('@librechat/api');
const {
  replaceSpecialVars,
  EModelEndpoint,
  Constants,
...
const getScheduledTaskAgents = (client) => {
  const agents = [];

  if (client?.options?.agent) {
    agents.push(client.options.agent);
  }

  if (client?.agentConfigs?.size > 0) {
...
const reconnectScheduledTaskMCPServers = async ({
  user,
  agents,
  signal,
  requestBody,
  ...
  const oauthServers = await getMCPServersRegistry().getOAuthServers(user.id);
  ...
  if (failures.length > 0) {
    const failureSummary = failures.map(({ serverName, message }) => `${serverName}: ${message}`);
    throw new Error(
      `OAuth MCP reconnection failed for scheduled task: ${failureSummary.join('; ')}`,
    );
  }
};
...
const executeScheduledTask = async ({ task, run, user, appConfig, maxRuntimeSeconds }) => {
  ...
  try {
    ...
    const taskAgents = getScheduledTaskAgents(client);
    if (taskAgents.length > 0) {
      const flowManager = getFlowStateManager(getLogStores(CacheKeys.FLOWS));
      await reconnectScheduledTaskMCPServers({
        user,
        agents: taskAgents,
        flowManager,
