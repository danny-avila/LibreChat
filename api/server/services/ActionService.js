...
const { GraphEvents, sleep } = require('@brainiac/agents');
const { logger, encryptV2, decryptV2 } = require('@brainiac/data-schemas');
const {
  sendEvent,
  logAxiosError,
  refreshAccessToken,
...
async function createActionTool({
  ...
  const _call = async (toolInput, config) => {
    try {
      /** @type {import('brainiac-data-provider').ActionMetadataRuntime} */
      const metadata = action.metadata;
