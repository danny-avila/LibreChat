const fs = require('fs');
const initializeOpenAI = require('./initialize');
 ...
 */
async function uploadOpenAIFile(req, file) {
  try {
    const openai = initializeOpenAI(req);
 ...
 */
async function deleteOpenAIFile(req, file) {
  try {
    const openai = initializeOpenAI(req);
