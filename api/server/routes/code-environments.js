...
const { getAppConfig, clearOverrideCache } = require('~/server/services/Config');
const { requireJwtAuth } = require('~/server/middleware');

const router = express.Router();
const registry = createCodeEnvironmentRegistry(mongoose);
const handlers = createCodeEnvironmentHttpHandlers({ getAppConfig, clearOverrideCache, registry });
