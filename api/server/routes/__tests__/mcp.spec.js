...
const { getBasePath } = require('@brainiac/api');
...
jest.mock('@brainiac/api', () => {
  const actual = jest.requireActual('@brainiac/api');
  return {
    ...actual,
    MCPOAuthHandler: {
    ...
    // Error handling utilities (from @brainiac/api mcp/errors)
...
describe('MCP Routes', () => {
  ...
  describe('GET /:serverName/oauth/initiate', () => {
    const { MCPOAuthHandler } = require('@brainiac/api');
  ...
  describe('GET /:serverName/oauth/callback', () => {
    const { MCPOAuthHandler, MCPTokenStorage } = require('@brainiac/api');
    const { getLogStores } = require('~/cache');
  ...
  describe('POST /:serverName/reinitialize', () => {
    ...
    it('should handle server with custom user variables', async () => {
      ...
      require('~/config').getFlowStateManager.mockReturnValue({});
      require('~/cache').getLogStores.mockReturnValue({});
      require('@brainiac/api').getUserMCPAuthMap.mockResolvedValue({
        'mcp:test-server': {
      ...
      expect(require('@brainiac/api').getUserMCPAuthMap).toHaveBeenCalledWith({
        userId: 'test-user-id',
        servers: ['test-server'],
