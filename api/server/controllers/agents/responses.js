...
const collectContextAgents = (rootAgents) => {
  const contextAgents = [];
  const pending = [...rootAgents];
  const visitedIds = new Set();

  for (let index = 0; index < pending.length; index++) {
    const agent = pending[index];
    if (!agent?.id || visitedIds.has(agent.id)) {
      continue;
    }
    visitedIds.add(agent.id);
    contextAgents.push(agent);
...
const createResponse = async (req, res) => {
  ...
  try {
    ...
    const contextAgents = collectContextAgents(runAgents);
