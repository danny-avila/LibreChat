...
const OpenAIChatCompletionController = async (req, res) => {
  ...
  try {
    ...
    // Tool calls from on_run_step, keyed by tool id, held until a delta claims
    // them. Providers that stream tool_call deltas (OpenAI-compatible) own the
    // index and consume these; providers that don't (Google/Vertex) leave them
    // here to be flushed once the run ends.
