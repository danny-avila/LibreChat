...
const { saveMessage, getMessages, getConvo, isUserDeleting } = require('~/models');
const {
...
const ResumableAgentController = async (req, res, next, initializeClient, addTitle) => {
  ...
  try {
    ...
    // The job is durable and active-set visible: the admission lease has done its work.
    releaseAdmissionLease();
    ...
    startGeneration().catch(async (err) => {
      ...
      if (!pausePersistenceFailed) {
        // Same fail-closed rule as the main catch: no terminal CAS without a fence.
        if (await registerUserFinalizationFence()) {
          ...
          logger.error(
            `[ResumableAgentController] Leaving ${streamId} active for the stale reaper — ` +
              'background-error terminal could not be fenced against account deletion',
          );
        }
      }
      await awaitPendingPersistence().catch(() => undefined);
