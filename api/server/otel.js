...
/** Kept in a function rather than guarding with a top-level `return`: Node
 *  allows that in CJS, but Babel (jest) parses this file as a module and
 *  rejects it, which breaks any suite that loads `~/server/index`. */
function startTelemetry() {
  ...
  try {
    ...
    // Attach after a tick so the app's winston logger exists first
    setImmediate(() => {
      try {
