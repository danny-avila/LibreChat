...
const { EModelEndpoint, Constants, openAISettings, CacheKeys } = require('brainiac-data-provider');
...
function getImporter(jsonData) {
  ...
  if (jsonData.conversationId && (jsonData.messagesTree || jsonData.messages)) {
    logger.info('Importing Brainiac conversation');
    return importBrainiacConvo;
  }
...
/**
 * Imports a Brainiac conversation from JSON.
 *
 ...
 * @returns {Promise<void>} - A promise that resolves when the import is complete.
 */
async function importBrainiacConvo(
  jsonData,
  requestUserId,
  builderFactory = createImportBatchBuilder,
) {
  try {
    ...
    if (jsonData.recursive) {
      ...
      throw new Error('Invalid Brainiac file format');
    }
    ...
    logger.error(`user: ${requestUserId} | Error creating conversation from Brainiac file`, error);
  }
}
