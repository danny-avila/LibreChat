...
describe('User Group Methods Tests', () => {
  ...
  describe('User-Group Relationship Methods', () => {
    ...
    test('should add user to group', async () => {
      ...
      const userIdOnTheSource = result.user.idOnTheSource || testUser1._id.toString();
    ...
    test('should return user and public principals for non-existent user in getUserPrincipals', async () => {
      ...
      expect(principals[1].principalId).toBeNull();
    });
  });
  ...
  describe('Entra ID Synchronization', () => {
    ...
    test.skip('should sync Entra groups for a user (add new groups)', async () => {
      ...
      for (const group of groups) {
        expect(group.memberIds).toContain(testUser.idOnTheSource || testUser._id.toString());
      }
    });
    ...
    test.skip('should sync Entra groups for a user (add and remove groups)', async () => {
      ...
      expect(newGroup.memberIds).toContain(testUser.idOnTheSource || testUser._id.toString());
    });
    ...
    test.skip('should preserve local groups when syncing Entra groups', async () => {
      ...
      const localGroup = await Group.create({
        name: 'Local Group',
        source: 'local',
        memberIds: [testUser.idOnTheSource || testUser._id.toString()]
      });
      ...
      expect(savedLocalGroup.memberIds).toContain(testUser.idOnTheSource || testUser._id.toString());
      ...
      expect(entraGroup.memberIds).toContain(testUser.idOnTheSource || testUser._id.toString());
    });
  });
});