...
describe('AclEntry Model Tests', () => {
  ...
  describe('Permission Grant and Query', () => {
    ...
    test('should find entries by principal', async () => {
      ...
      await grantPermission('user', userId, 'agent', resourceId, PermissionBits.VIEW, grantedById);
      await grantPermission('user', userId, 'project', new mongoose.Types.ObjectId(), PermissionBits.EDIT, grantedById);
      
      // Find all entries for the user
      const entries = await findEntriesByPrincipal('user', userId);
      expect(entries).toHaveLength(2);
      
      // Find entries filtered by resource type
      const agentEntries = await findEntriesByPrincipal('user', userId, 'agent');
    ...
    test('should find entries by resource', async () => {
      ...
      await grantPermission('user', userId, 'agent', resourceId, PermissionBits.VIEW, grantedById);
      await grantPermission('group', groupId, 'agent', resourceId, PermissionBits.EDIT, grantedById);
      await grantPermission('public', null, 'agent', resourceId, PermissionBits.VIEW, grantedById);
      
      const entries = await findEntriesByResource('agent', resourceId);
  ...
  describe('Permission Checks', () => {
    beforeEach(async () => {
      ...
      await grantPermission('public', null, 'agent', otherResourceId, PermissionBits.VIEW, grantedById);
    });
    ...
    test('should find entries by principals and resource', async () => {
      ...
      const entries = await findEntriesByPrincipalsAndResource(principalsList, 'agent', resourceId);
      expect(entries).toHaveLength(2);
    });
    ...
    test('should check if user has permission', async () => {
      ...
      const hasViewPermission = await hasPermission(principalsList, 'agent', resourceId, PermissionBits.VIEW);
      expect(hasViewPermission).toBe(true);
      
      // User doesn't have EDIT permission
      const hasEditPermission = await hasPermission(principalsList, 'agent', resourceId, PermissionBits.EDIT);
    ...
    test('should check permission for multiple principals', async () => {
      ...
      const hasDeletePermission = await hasPermission(principalsList, 'agent', resourceId, PermissionBits.DELETE);
    ...
    test('should get effective permissions', async () => {
      ...
      const effective = await getEffectivePermissions(principalsList, 'agent', resourceId);
  ...
  describe('Permission Modification', () => {
    test('should revoke permission', async () => {
      ...
      // Check it exists
      const entriesBefore = await findEntriesByPrincipal('user', userId);
      expect(entriesBefore).toHaveLength(1);
      
      // Revoke it
      const result = await revokePermission('user', userId, 'agent', resourceId);
      expect(result.deletedCount).toBe(1);
      
      // Verify it's gone
      const entriesAfter = await findEntriesByPrincipal('user', userId);
    ...
    test('should modify permission bits - add permissions', async () => {
      ...
      // Add EDIT permission
      const updated = await modifyPermissionBits(
        'user',
        userId,
        'agent',
        resourceId,
  ...
  describe('Resource Access Queries', () => {
    test('should find accessible resources', async () => {
      ...
      await grantPermission('user', userId, 'agent', resourceId1, PermissionBits.VIEW, grantedById);
      
      // User can view and edit resource 2
      await grantPermission('user', userId, 'agent', resourceId2, PermissionBits.VIEW | PermissionBits.EDIT, grantedById);
      
      // Group can view resource 3
      await grantPermission('group', groupId, 'agent', resourceId3, PermissionBits.VIEW, grantedById);
      
      // Find resources with VIEW permission for user
      const userViewableResources = await findAccessibleResources(
        [{ principalType: 'user', principalId: userId }],
        'agent',
        PermissionBits.VIEW
      );
      ...
      const allViewableResources = await findAccessibleResources(
        [
          { principalType: 'user', principalId: userId },
          { principalType: 'group', principalId: groupId }
        ],
    ...
    test('should handle inherited permissions', async () => {
      ...
      await grantPermission('user', userId, 'project', projectId, PermissionBits.VIEW, grantedById);
