...
class AnthropicClient extends BaseClient {
  constructor(apiKey, options = {}) {
    ...
    this.streamHandler;
    /** File IDs of files generated by Anthropic Skills / code execution.
     *  Populated by sendCompletion's stream loop, consumed by the post-stream
     *  attachment fetch (see fetchGeneratedFiles).
     *  @type {Array<{ file_id: string, filename?: string }>} */
    this.generatedFiles = [];
  }
  ...
  getClient(requestOptions) {
    ...
    const useSkills =
      ...
      requestOptions.tools.some((tool) => tool?.name === 'code_execution');
  ...
  /**
   * Inspect a single Anthropic stream event for file_ids generated by the
   * code execution tool or Skills, and append any matches to
   * this.generatedFiles. Defensive: logs unfamiliar block types instead of
   * throwing so a shape we don't recognize doesn't break the stream loop.
   * @param {object | undefined} event
   */
  captureGeneratedFileIds(event) {
    ...
    if (
      ...
      const nested = inner && Array.isArray(inner.content) ? inner.content : [];
      for (const item of nested) {
        if (item && typeof item === 'object' && typeof item.file_id === 'string') {
  ...
  async sendCompletion(payload, { onProgress, abortController }) {
    ...
    if (
      ...
      const skillsDirective =
        ...
        'you may briefly say what you are creating (e.g. "Creating your document...") ' +
        'but do NOT reveal internal mechanics. Never mention "skills", "skill files", ' +
        '"SKILL.md", "code execution", "container", "Python", or any tool/library names. ' +
        'Do not narrate intermediate steps like reading instructions, installing libraries, ' +
        'or building the file. The file attachment is the primary output.';

      if (Array.isArray(requestOptions.system)) {
        requestOptions.system = [
          ...requestOptions.system,
          { type: 'text', text: skillsDirective },
        ];
      } else if (typeof requestOptions.system === 'string') {
        requestOptions.system = [
          { type: 'text', text: requestOptions.system },
          { type: 'text', text: skillsDirective },
        ];
      } else {
        requestOptions.system = [{ type: 'text', text: skillsDirective }];
      }
    }
      ...
      while (attempts < maxRetries) {
        ...
        try {
          ...
          for await (const completion of response) {
            ...
            this.captureGeneratedFileIds(completion);
      ...
      for (const fileEntry of this.generatedFiles) {
        this.artifactPromises.push(
          processAnthropicFile({
            ...
            filenameHint: fileEntry.filename,
            conversationId: this.conversationId,
