  ...
  if (tools && tools.length > 0 && user) {
    for (const toolKey of tools) {
      if (toolKey && appTools[toolKey] && mcpToolPattern.test(toolKey)) {
        ...
        const mcpPluginKey = `mcp_${serverName}`;
...
`.trim();
      ...
      // Log a warning if we expected to find vars for this key (because we fetched for it) but it's empty.
      if (
        mcpPluginKeysToFetch.includes(mcpPluginKey) &&
        Object.keys(customUserVars).length === 0
      ) {
        logger.warn(
          `[handleTools] No customUserVars found for MCP tool ${mcpPluginKey} and user ${user} after batch fetch. Proceeding without customUserVars.`,
        );
      }
