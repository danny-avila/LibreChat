...
class NurieAIClient extends BaseClient {
  constructor(apiKey, options = {}) {
    super(apiKey, options);
    this.apiKey = apiKey || process.env.NURIEAI_API_KEY;
    this.baseUrl = process.env.NURIEAI_HOST;
    // let creds = {};
  ...
  // constructUrl() {
  //   console.log('constructUrl NurieAI')
  ...
  setOptions(options) {
    ...
    this.sender =
      ...
      getResponseSender({
        ...
        endpoint: EModelEndpoint.nurieAI,
  ...
  async buildMessages(
    ...
    // });
    return { prompt: messages[messages.length - 1] };
  }
  ...
  async getCompletion(payload, abortController = null) {
    ...
    const orgination = this.options.req.user.orgination;
    ...
    const res = await axios.post(url.toString(), body, options);
  ...
  getBuildMessagesOptions() {
    logger.debug('NurieAI client doesn\'t use getBuildMessagesOptions');
  }
