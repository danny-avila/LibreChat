...
// Define the shape of the message type
interface MessageType {
  type: 'json' | 'mixed';
  content: any; // For JSON, content is the parsed object; for mixed, it's an array of segments
}

// Utility function to split and detect message segments
...
// Utility function to detect and handle JSON
...
// Utility function to render JSON in a human-readable format
const renderJsonContent = (json: any) => {
        ...
        {json.clarification_question.options && (
          <div>
            ...
            <ul className="list-disc pl-5">
              {json.clarification_question.options.map((option: { title: string; description: string }, index: number) => (
                <li key={index}>
  ...
  return <pre className="text-sm">{JSON.stringify(json, null, 2)}</pre>;
};

// Define the shape of the log object (adjust based on your actual data structure)
interface QueryLog {
  _id: string;
...
const QueryLogDetailsDialog: React.FC<QueryLogDetailsDialogProps> = ({ dialogOpen, setDialogOpen, selectedLog }) => {
        ...
        {selectedLog && (
          <div className="space-y-6 p-4 text-sm text-foreground">
            ...
            <div>
              ...
              <div className="max-h-60 overflow-y-auto rounded-md border bg-muted p-3 text-sm text-muted-foreground dark:bg-muted/50">
                      ...
                      <div key={index} className="mb-2">
                        ...
                        {segment.type === 'code' && (
                          <SyntaxHighlighter
                            ...
                            style={vscDarkPlus}
                            customStyle={{ margin: 0, padding: '0.5rem', fontSize: '0.875rem' }}
                          >
                            {segment.content}
                          </SyntaxHighlighter>
                        )}
