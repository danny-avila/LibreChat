...
export default function FileViewer({
  ...
  useEffect(() => {
    if (pdfRef.current && currentPage && fileType.includes('pdf')) {
      loadPage(pdfRef.current, currentPage);
    }
  }, [scale, rotation, loadPage, currentPage, fileType]);
    ...
    return (
      <div className={cn('flex h-full items-center justify-center', className)}>
        <div className="text-center">
          ...
          <p className="text-text-secondary">Loading {fileType.includes('pdf') ? 'PDF' : 'file'}...</p>
        </div>
      </div>
    );
  }
  ...
  // For non-PDF files, show a simple file viewer
  if (!fileType.includes('pdf')) {
    return (
      <div className={cn('flex h-full flex-col bg-white', className)}>
        ...
        <div className="flex-1 overflow-auto p-4">
          <div className="text-center">
            <p className="text-text-secondary mb-4">File type: {fileType}</p>
            <button
              onClick={downloadFile}
              className="rounded bg-blue-500 px-4 py-2 text-white hover:bg-blue-600"
            >
