...
describe('Error — assistant tool permission', () => {
  const refused = (tools?: string[]) => ({
    type: ErrorTypes.ASSISTANT_TOOL_NOT_PERMITTED,
    ...(tools ? { tools } : {}),
  });

  it('names the denied tool with the label the assistant builder shows', () => {
    renderError(refused(['code_interpreter']), providerMessage);

    expect(
      screen.getByText(
        localized(
          'com_error_assistant_tool_not_permitted',
  ...
  it('lists each denied tool once', () => {
    renderError(refused(['code_interpreter', 'file_search', 'code_interpreter']), providerMessage);
  ...
  it('falls back to a generic sentence when the payload names no tools', () => {
    renderError(refused(), providerMessage);

    expect(
      screen.getByText(catalog.com_error_assistant_tool_not_permitted_generic),
    ).toBeInTheDocument();
    expectReadable();
  });
});
