...
const MinimalIcon: React.FC<IconProps> = (props) => {
  ...
  return (
    <div
      ...
      className={cn(
        // 👇 apply circular border & background
        'shadow-stroke relative flex items-center justify-center rounded-full bg-white dark:bg-presentation text-black dark:text-white',
        props.className ?? '',
      )}
    >
      {icon}
