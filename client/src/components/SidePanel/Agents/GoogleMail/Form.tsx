...
export default function GoogleMailForm() {
    ...
    <div className="flex items-center">
      <Controller
        ...
        render={({ field }) => (
          <Checkbox
            {...field}
            id="google-mail-checkbox"
            checked={field.value === true}
            onCheckedChange={(checked) => field.onChange(checked === true)}
            className="relative float-left mr-2 inline-flex h-4 w-4 cursor-pointer"
            value={String(field.value)}
            aria-labelledby="google-mail-label"
          />
        )}
      />
      <label
        id="google-mail-label"
        htmlFor="google-mail-checkbox"
        className="form-check-label text-token-text-primary cursor-pointer text-sm"
      >
        {localize('com_ui_google_mail')}
      </label>
    </div>
  );
}
