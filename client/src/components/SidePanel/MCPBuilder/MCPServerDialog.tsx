...
import type { McpServerCreateParams, MCPServerDB } from 'librechat-data-provider';
import {
  OGDialog,
  OGDialogTemplate,
  OGDialogContent,
...
export default function MCPServerDialog({
    ...
    if (server) {
      ...
      return {
        ...
        auth: {
          ...
          server_id: server.mcp_id, // For edit mode redirect URI
        },
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteMutation.mutateAsync(server.mcp_id);
  ...
  const onSubmit = handleSubmit(async (formData: MCPServerFormData) => {
    ...
    try {
      ...
      const result = server
        ? await updateMutation.mutateAsync({ mcp_id: server.mcp_id, data: params })
        : await createMutation.mutateAsync(params);

      const serverId = result.mcp_id;
  ...
  const shouldShowShareButton =
    ...
    (server.author === user?.id || user?.role === SystemRoles.ADMIN || canShareThisServer) &&
