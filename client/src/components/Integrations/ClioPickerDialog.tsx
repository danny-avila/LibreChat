...
import { useClioDocumentsQuery } from '~/data-provider';
...
interface ClioPickerDialogProps {
  isOpen: boolean;
...
export function ClioPickerDialog({
  isOpen,
  onOpenChange,
  onFilesSelected,
  isAttaching = false,
  ...
  const { data, isLoading, isFetching, isError, error } = useClioDocumentsQuery({
  ...
  return (
    <IntegrationPickerDialogShell
      ...
      titleKey="com_integrations_clio_picker_title"
      searchPlaceholderKey="com_integrations_clio_picker_search"
      searchValue={search}
