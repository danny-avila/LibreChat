...
import { useMicrosoftOneDriveFilesQuery } from '~/data-provider';
import { useDebounce, useLocalize } from '~/hooks';
import { IntegrationPickerDialogShell } from './IntegrationPickerDialogShell';
import {
  isIntegrationReconnectApiError,
  isOneDriveNotProvisionedApiError,
} from '~/utils/integrationReconnect';

interface MicrosoftOneDrivePickerDialogProps {
  isOpen: boolean;
...
export function MicrosoftOneDrivePickerDialog({
  isOpen,
  onOpenChange,
  onFilesSelected,
  isAttaching = false,
  maxSelectionCount,
  onReconnect,
}: MicrosoftOneDrivePickerDialogProps) {
  ...
  const { data, isLoading, isFetching, isError, error } = useMicrosoftOneDriveFilesQuery({
  ...
  const showProvisionError = isError && isOneDriveNotProvisionedApiError(error);
  const showReconnectError =
    isError && !showProvisionError && isIntegrationReconnectApiError(error);

  return (
    <IntegrationPickerDialogShell
      isOpen={isOpen}
      onOpenChange={onOpenChange}
      titleKey="com_integrations_microsoft_onedrive_picker_title"
      searchPlaceholderKey="com_integrations_microsoft_onedrive_picker_search"
      searchValue={search}
      ...
      isError={showReconnectError || showProvisionError}
      errorDescriptionKey={
        showProvisionError ? 'com_integrations_onedrive_not_provisioned' : undefined
      }
      onReconnect={showReconnectError ? onReconnect : undefined}
