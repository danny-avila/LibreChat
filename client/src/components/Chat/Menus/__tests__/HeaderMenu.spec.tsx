...
describe('HeaderMenu', () => {
  ...
  it('offers the trace in its own group only when the header says one is available', () => {
    const { rerender } = render(
      <HeaderMenu onTagsUpdated={onTagsUpdated} trace={{ show: false, open: jest.fn() }} />,
    );
    expect(labels()).not.toContain('com_ui_trace_view');

    rerender(<HeaderMenu onTagsUpdated={onTagsUpdated} trace={{ show: true, open: jest.fn() }} />);
  ...
  it('keeps the trace reachable when it is the only action left', () => {
    ...
    render(<HeaderMenu onTagsUpdated={onTagsUpdated} trace={{ show: true, open: jest.fn() }} />);
