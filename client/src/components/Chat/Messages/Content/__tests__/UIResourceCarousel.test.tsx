...
jest.mock('~/hooks', () => ({
  useLocalize: () => (key: string) => {
    if (key === 'com_ui_scroll_left') {
      return 'Scroll left';
    }
    if (key === 'com_ui_scroll_right') {
      return 'Scroll right';
    }
    if (key === 'com_ui_mcp_app_shared_unavailable') {
      return "MCP Apps aren't viewable in shared conversations.";
    }
    return key;
  },
}));
...
describe('UIResourceCarousel', () => {
  ...
  it('filters unsupported resources before building app cards', () => {
    const blocked: UIResource = {
    ...
    const { container } = render(
      <UIResourceCarousel uiResources={[blocked, mockUIResources[0]]} />,
    );

    expect(container.querySelectorAll('iframe[data-sandbox-url]')).toHaveLength(1);
    expect(container.querySelectorAll('.hide-scrollbar > div')).toHaveLength(1);
  });

  it('renders nothing when every resource is unsupported', () => {
    const blocked: UIResource = {
      resourceId: 'blocked-resource',
      uri: 'ui://blocked',
    ...
    const { container } = render(<UIResourceCarousel uiResources={[blocked]} />);
