...
describe('ActivityPhaseGroup', () => {
  ...
  test('reports every toggle so the message can restore it', () => {
    const onExpansionChange = jest.fn();
    render(
      <ActivityPhaseGroup labelPart={labelPart} hasContent onExpansionChange={onExpansionChange}>
        <div data-testid="phase-content" />
      </ActivityPhaseGroup>,
    );

    fireEvent.click(screen.getByRole('button', { name: LABEL }));
    expect(onExpansionChange).toHaveBeenLastCalledWith({ isExpanded: true, userOverride: true });

    fireEvent.click(screen.getByRole('button', { name: LABEL }));
    expect(onExpansionChange).toHaveBeenLastCalledWith({ isExpanded: false, userOverride: true });
  });

  test('restores a stored toggle instead of replaying the entrance fold', () => {
    render(
      <ActivityPhaseGroup
        labelPart={labelPart}
        hasContent
    ...
    const trigger = screen.getByRole('button', { name: LABEL });
    expect(trigger).toHaveAttribute('aria-expanded', 'true');
    /** No entrance is queued, so the card cannot fold shut underneath a
     *  reader who opened it before the summary swapped in. */
    expect(pendingFrames()).toBe(0);
  ...
  test('ignores a stored state that was never a user choice', () => {
    render(
      <ActivityPhaseGroup
        labelPart={labelPart}
        hasContent
        initialExpansionState={{ isExpanded: true, userOverride: false }}
      >
        <div data-testid="phase-content" />
      </ActivityPhaseGroup>,
    );
