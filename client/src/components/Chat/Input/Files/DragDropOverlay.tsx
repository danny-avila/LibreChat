...
/**
 * ChatGPT-style full-screen drop overlay.
 * Rendered inside DragDropWrapper (the react-dnd drop target) so drop events
 * on the overlay still bubble to the wrapper.
 */
const DragDropOverlay = memo(({ isActive }: DragDropOverlayProps) => {
  ...
  return (
    <div
      ...
      className={`fixed inset-0 z-[9999] flex flex-col items-center justify-center transition-opacity duration-150 ease-in-out ${
        isActive
    ...
    >
      {/* 반투명 배경 레이어 — surface-primary 가 hex 변수라 Tailwind alpha 수식 대신 opacity 사용 */}
      <div className="absolute inset-0 bg-surface-primary opacity-95 backdrop-blur-sm" />
      <svg
      ...
      <h2 className="relative mt-6 text-xl font-bold text-text-primary">
        {localize('com_ui_drop_here_title')}
      </h2>
      <p className="relative mt-1 text-sm text-text-secondary">
        {localize('com_ui_drop_here_subtitle')}
      </p>
    </div>
  );
});
