...
export default function ModelMenu() {
  ...
  console.log({ model, promptPrefix, chatGptLabel });
  ...
  // fetch filters to know which key is configured
  useEffect(() => {
    axios
      .get("/api/models", {
        timeout: 1000,
  ...
  useEffect(() => {
    if (model)
      localStorage.setItem(
        "model",
        JSON.stringify({ model, chatGptLabel, promptPrefix })
      );
  }, [model]);
        ...
        >
          ...
          >
            {availableModels.length ? (
              <MenuItems models={availableModels} onSelect={onChange} />
            ) : (
              <DropdownMenuLabel className="dark:text-gray-300">
                No model available.
              </DropdownMenuLabel>
