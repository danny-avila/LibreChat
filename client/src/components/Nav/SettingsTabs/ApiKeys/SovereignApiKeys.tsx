...
import {
  useGetSovereignKeysQuery,
  useCreateSovereignKeyMutation,
  useDeleteSovereignKeyMutation,
...
function CreateKeyDialog({ onKeyCreated }: { onKeyCreated?: () => void }) {
  ...
  const createMutation = useCreateSovereignKeyMutation();
...
function KeyItem({
  ...
  const deleteMutation = useDeleteSovereignKeyMutation();
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <div className="text-sm text-text-secondary">
            ...
            <span>
              {localize('com_ui_created')} {formatDate(created_at)}
            </span>
            {last_used && (
              <>
                <span className="mx-2">•</span>
                <span>
                  {localize('com_ui_last_used')} {formatDate(last_used)}
                </span>
              </>
            )}
          </div>
...
function ApiKeysContent({ isOpen }: { isOpen: boolean }) {
  const localize = useLocalize();
  const { data, isLoading, error } = useGetSovereignKeysQuery({ enabled: isOpen });
...
export function SovereignApiKeys() {
  const localize = useLocalize();
  const [isOpen, setIsOpen] = useState(false);

  return (
    <div className="flex items-center justify-between">
      <Label id="sovereign-api-keys-label">{localize('com_ui_sovereign_api_keys')}</Label>

      <OGDialog open={isOpen} onOpenChange={setIsOpen}>
        <OGDialogTrigger asChild>
          <Button aria-labelledby="sovereign-api-keys-label" variant="outline">
            {localize('com_ui_manage')}
          </Button>
        </OGDialogTrigger>

        <OGDialogContent
          title={localize('com_ui_sovereign_api_keys')}
          className="w-11/12 max-w-2xl bg-background text-text-primary shadow-2xl"
        >
          <OGDialogHeader>
            <OGDialogTitle>{localize('com_ui_sovereign_api_keys')}</OGDialogTitle>
            <p className="text-sm text-text-secondary">
              {localize('com_ui_sovereign_api_keys_description')}
            </p>
          </OGDialogHeader>
