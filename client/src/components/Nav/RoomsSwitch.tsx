...
export default function RoomsSwitch({ roomSearchIndex, setRoomSearchIndex }) {

  return (
    <Tab.Group
      onChange={(i) => setRoomSearchIndex(i === 1 ? 'all' : 'user')}
      selectedIndex={roomSearchIndex === 'user' ? 0 : 1}
    >
      <Tab.List className="mt-1 flex space-x-1 rounded-xl bg-gray-200 p-1 outline-none dark:bg-gray-750">
        <Tab
          key="Rooms"
          value="r"
          className={({ selected }) =>
