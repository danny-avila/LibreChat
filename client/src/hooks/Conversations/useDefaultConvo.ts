...
import { excludedKeys, getDefaultParamsEndpoint, resolveModelCatalogKey } from 'librechat-data-provider';
import type {
  TEndpointsConfig,
  TModelsConfig,
  TConversation,
...
const useDefaultConvo = () => {
  ...
  const getDefaultConversation = useCallback(
    ({ conversation: _convo, preset, cleanInput, cleanOutput }: TDefaultConvo) => {
      ...
      const models = modelsConfig[resolveModelCatalogKey(endpoint)] || [];
