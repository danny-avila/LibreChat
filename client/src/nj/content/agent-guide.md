...
Like chats, you use an agent in a chat conversation and can direct the work as needed. For example, an Email Drafter agent can draft a generic email, which you then customize through follow-up prompts or feedback. Alternatively, create separate agents for different email types you write regularly. See "*How do I write an effective agent?*" for how to build an agent.
...
* **Important note: Start every agent with the instruction “You are the \[agent name\], which helps to \[short description\].”** Without this context to start the agent instructions, you will not be able to activate agents mid-conversation effectively, as the agent will not have enough context to work. 
