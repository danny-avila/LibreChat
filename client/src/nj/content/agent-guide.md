...
**Agents are a tool for repetitive tasks or skills you regularly use in your work.**
...
Like chats, you use an agent in a chat conversation and can direct the work as needed. For example, an Email Drafter agent can draft a generic email, which you then customize through follow-up prompts or feedback. Alternatively, create separate agents for different email types you write regularly. See _"How do I write an effective agent?"_ for how to build an agent.

## **What agents can I use?**

Access agents built by the NJ AI Assistant team or yourself:

- **Use a pre-built agent in the Agent Library:** the NJ AI Assistant team created agents for common use cases: Email Drafter, Meeting Summarizer, Proofreader & Polisher, Prompt Improver, and Brainstorming Buddy. Find these in “Agent Library” at the top of the left sidebar. Click on one to start using it!
...
- **Customize a pre-built agent:** Duplicate a pre-built agent and tailor the instructions. For example, you can take the pre-built Email Drafter, duplicate it, then add email examples and instructions to reflect your writing style and tone. See “How can I duplicate an agent?” for details.
...
## **How can I deactivate or switch agents in a conversation?**

**The agent name appears in the selector at the top when active, providing an easy way to deactivate or switch agents.**
...
Writing a good agent is similar to writing a good prompt, here’s some best practices.
...
### **Agent Handoffs (in Advanced Settings)**

This will connect the current agent to other agents to do a task, having the other agent act as a specialist. The core agent acts as a coordinator, and uses other agents that are relevant.
...
When doing more complex tasks, through “advanced settings” at the bottom of the agent panel, you can use the “agent handoff.” This will connect multiple agents who do different tasks. Specifically, it will shift the task from one agent to another with a specific specialty. In the example above, you would connect the multiple agents mentioned here: one for identifying common themes, one for honing themes, one for labeling the comments with a theme, and one to write the summary of top themes.
