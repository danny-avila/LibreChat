...
const dateKeysReverse = Object.fromEntries(Object.entries(dateKeys).map(([key, value]) => [value, key]));
...
export const addConversation = (data, newConversation) => {
...
export function findPageForConversation(data, conversation) {
    let pageIndex = -1;
    let index = -1;
    data.pages.forEach((page, pIdx) => {
        const idx = page.conversations.findIndex((c) => c.conversationId === conversation.conversationId);
        if (idx !== -1) {
            pageIndex = pIdx;
            index = idx;
        }
...
export const updateConversation = (data, updatedConversation) => {
    const newPages = data.pages.map((page) => ({
        ...page,
        conversations: page.conversations.map((convo) => convo.conversationId === updatedConversation.conversationId ? updatedConversation : convo),
    }));
    return { ...data, pages: newPages };
};
export const updateConvoFields = (data, updatedConversation, keepPosition = false) => {
    const newData = JSON.parse(JSON.stringify(data));
    const { pageIndex, index } = findPageForConversation(newData, updatedConversation);
...
export const deleteConversation = (data, conversationId) => {
    const newPages = data.pages.map((page) => ({
        ...page,
...
export const getConversationById = (data, conversationId) => {
