...
import { dateKeys, addConversation, updateConvoFields, updateConversation, deleteConversation, findPageForConversation, groupConversationsByDate, } from './convos';
...
describe('Conversation Utilities', () => {
    describe('groupConversationsByDate', () => {
        ...
        it('groups conversations correctly across multiple years', () => {
            ...
            const getGroupLength = (key) => grouped.find(([k]) => k === key)?.[1]?.length ?? 0;
            ...
            const totalGroupedConversations = grouped.reduce((total, [, convos]) => total + convos.length, 0);
    ...
    describe('addConversation', () => {
        it('adds a new conversation to the top of the list', () => {
            ...
            const newData = addConversation(data, newConversation);
    ...
    describe('updateConversation', () => {
        it('updates an existing conversation and moves it to the top', () => {
            ...
            const newData = updateConversation(initialData, updatedConversation);
    ...
    describe('updateConvoFields', () => {
        it('updates specific fields of a conversation', () => {
            ...
            const newData = updateConvoFields(initialData, updatedFields);
    ...
    describe('deleteConversation', () => {
        it('removes a conversation by id', () => {
            ...
            const newData = deleteConversation(initialData, '1');
            expect(newData.pages[0].conversations).toHaveLength(1);
    ...
    describe('findPageForConversation', () => {
        it('finds the correct page and index for a given conversation', () => {
            ...
            const { pageIndex, index } = findPageForConversation(data, {
                conversationId: '2',
            });
...
describe('Conversation Utilities with Fake Data', () => {
    ...
    describe('updateConvoFields', () => {
        it('updates specific fields of a conversation in fake data', () => {
            ...
            const newData = updateConvoFields(convoData, updatedFields);
            const updatedConversation = newData.pages[0].conversations.find((c) => c.conversationId === updatedFields.conversationId);
            expect(updatedConversation?.title).toBe('Partially Updated Title');
        });
    });
    describe('deleteConversation', () => {
        it('removes a conversation by id from fake data', () => {
            const conversationIdToDelete = convoData.pages[0].conversations[0].conversationId;
            const newData = deleteConversation(convoData, conversationIdToDelete);
            const deletedConvoExists = newData.pages[0].conversations.some((c) => c.conversationId === conversationIdToDelete);
