# DNS Configuration for Brainiac Helm Chart

This feature allows you to configure custom DNS settings for Brainiac pods, enabling traffic redirection to proxy servers or custom endpoints.
...
Deploy:
```bash
helm upgrade --install brainiac ./helm/brainiac -f values.yaml
```
...
```bash
helm install brainiac ./helm/brainiac \
  --set dnsPolicy="None" \
...
✅ **Integration Test**
- Works with existing Brainiac configuration
...
- Kubernetes 1.19+
- Compatible with all Brainiac deployment modes
