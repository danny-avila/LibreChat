  ...
  async function getConvosByCursor(
    ...
    try {
      ...
      const convos = await Conversation.find(query)
        ...
        .select(
          'conversationId endpoint title createdAt updatedAt archivedAt isArchived user model agent_id assistant_id spec iconURL chatProjectId pinned lastResponseAt lastSeenAt',
        )
        .sort(sortObj)
        .limit(pageSize + 1)
        .lean<IConversation[]>();
