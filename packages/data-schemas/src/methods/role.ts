...
export function createRoleMethods(
  ...
  async function initializeRoles(): Promise<void> {
    ...
    for (const roleName of [SystemRoles.ADMIN, SystemRoles.USER]) {
      ...
      // Role is tenant-scoped on {name, tenantId} and this runs as system, so a name filter
      // alone matches an arbitrary tenant's document; migrate every one, keyed by _id.
