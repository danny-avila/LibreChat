...
/**
 * A suite's database addressed as one of the provisioned roles.
 *
 * Distinct from `SET LOCAL ROLE`: this actually authenticates, which is the only
 * way to show that a role the migrations created can log in with the password the
 * runner applied.
 */
export function roleDatabaseUrl(name: string, role: string, password: string): string {
  const url = new URL(isolatedDatabaseUrl(name));
