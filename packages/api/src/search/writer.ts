...
/**
 * Every write in this module runs under one lock and three fences:
 *
 *  - the projector lease epoch (`assertLeaseEpoch`), so a deposed holder cannot
 *    commit;
 *  - a per-record transaction-scoped advisory lock, so two concurrent writes for
 *    the same record serialize rather than interleave;
 *  - a **source-read fence**, so the newest source state wins whichever pass
 *    commits last. The lock alone cannot deliver that: the drain, the safety poll
 *    and the reconciliation backfill each read the source *before* opening the
 *    transaction that takes the lock, and they run on independent timers, so one
 *    pass can read old content while another reads and projects a newer edit, and
 *    the first can then take the lock and commit over it with a higher projection
 *    version. The source store has no per-record version to arbitrate with —
 *    `findOneAndUpdate` never bumps `__v` and `optimisticConcurrency` is
 *    configured nowhere — so the arbiter is *when the source was read*, carried on
 *    every write and compared in the `ON CONFLICT` guard;
 *  - a projection-version guard, so an out-of-order write still loses even if the
 *    others were somehow bypassed.
 */
 ...
 * projector-assigned version downstream, which is what lets a downstream consumer
 * replay idempotently.
 *
 * Three things make the `ON CONFLICT` guard refuse a write, and each of them
 * withholds the outbox row too, because the CTE it is selected from returns
 * nothing:
 *
 *  - a source read older than the one already projected. The newest source state
 *    wins regardless of which pass commits last.
 *  - a projection version below the stored one, for an out-of-order write.
 *  - **content identical to what is already stored.** Without this the safety
 *    poll's deliberate trailing-window re-read bumps the version of the same rows
 *    every minute forever — on a completely idle database — and appends an outbox
 *    row each time. `content_hash` covers every projected column, so an unchanged
...
export async function upsertDocument(
  ...
  space: string,
  /**
   * When the source state in `source` was read. Defaults to now for callers that
   * read and write in one step; the projector passes the instant it actually
   * started reading, which is the whole point of the fence.
   */
  sourceReadAt: Date = new Date(),
    ...
    `WITH v AS (SELECT nextval('chat_search.projection_version_seq') AS version),
     ...
     ins AS (
       ...
       ON CONFLICT (tenant_id, user_id, kind, record_id) DO UPDATE SET
         ...
         deleted_at = NULL,
         updated_at = now()
       WHERE excluded.source_read_at >= chat_search.documents.source_read_at
         AND excluded.projection_version > chat_search.documents.projection_version
         AND (chat_search.documents.content_hash <> excluded.content_hash
              OR chat_search.documents.deleted_at IS NOT NULL)
 ...
 *
 * The source-read fence applies here too, and for the same reason: an absence
 * observed by an earlier pass must not bury a later pass's upsert. A tombstone
 * derived from a stale read of the source is exactly as wrong as a stale upsert.
 */
export async function tombstoneDocument(
  ...
  const { rows } = await client.query<{ projection_version: string }>(
    ...
    [scoped.tenantId, scoped.userId, scoped.kind, scoped.recordId, now, sourceReadAt],
  );
...
/**
 * A projected key plus the hash of what is stored under it.
 *
 * The hash rides along because reconciliation already reads the source for every
 * window it walks: comparing it in memory turns the same pass into a repair for
 * rows whose *content* drifted, at no extra read. Without it reconciliation only
 * ever adds and removes keys, and a re-import that overwrites an existing
 * conversation with a historic `updatedAt` — behind the forward poll cursor, and
 * present in the projection so no backfill claims it — leaves the old title and
 * body searchable indefinitely.
 */
export type ProjectedKey = SearchRecordKey & { contentHash: string };
...
export async function scanProjectedKeys(
  ...
  const { rows } = await client.query<{
    ...
    `SELECT tenant_id, user_id, record_id, content_hash
