...
describePg('projector write path', () => {
  ...
  /**
   * The invariant the per-record lock cannot deliver on its own.
   *
   * Every pass reads the source *before* opening the transaction that takes the
   * lock, and the drain, the poll and the reconciliation backfill run on
   * independent timers — so a pass holding an older snapshot can take the lock
   * afterwards and commit over a newer edit with a higher projection version.
   * These interleave two passes explicitly, in both arrival orders.
   */
  describe('source-read fence', () => {
    const earlier = new Date('2026-03-01T10:00:00Z');
    const later = new Date('2026-03-01T10:00:05Z');

    it('refuses a stale snapshot that commits after a newer one', async () => {
      await withTransaction(pool, (client) =>
        upsertDocument(
          client,
          lease.epoch,
          sourceOf({ body: 'the edited body' }),
          DEFAULT_EMBEDDING_SPACE,
          later,
        ),
      );
      ...
      const stale = await withTransaction(pool, (client) =>
        upsertDocument(
          client,
          lease.epoch,
          sourceOf({ body: 'the body before the edit' }),
          DEFAULT_EMBEDDING_SPACE,
          earlier,
        ),
      );
      ...
      const { rows } = await pool.query<{ body: string }>('SELECT body FROM chat_search.documents');
      expect(rows[0].body).toBe('the edited body');
    });

    it('applies the newer snapshot when it arrives second', async () => {
      await withTransaction(pool, (client) =>
        upsertDocument(
          client,
          lease.epoch,
      ...
      const fresh = await withTransaction(pool, (client) =>
        upsertDocument(
          client,
          lease.epoch,
          sourceOf({ body: 'the edited body' }),
    ...
    /** A tombstone from a stale read must not bury a newer upsert either. */
    it('refuses a tombstone derived from a stale read', async () => {
      await withTransaction(pool, (client) =>
        upsertDocument(client, lease.epoch, sourceOf(), DEFAULT_EMBEDDING_SPACE, later),
      );

      const buried = await withTransaction(pool, (client) =>
        tombstoneDocument(client, lease.epoch, KEY, new Date(), earlier),
      );

      expect(buried.applied).toBe(false);
  ...
  /**
   * The safety poll re-reads its trailing lookback window on every pass, by
   * design. Without a content guard that re-read bumps the version of the same
   * rows every minute forever — on an idle database — and appends an outbox row
   * each time, for a table this change ships no consumer or retention for.
   */
  describe('unchanged upserts', () => {
    ...
    it('writes no outbox row and no new version for an identical projection', async () => {
      const first = await withTransaction(pool, (client) =>
        upsertDocument(client, lease.epoch, sourceOf(), DEFAULT_EMBEDDING_SPACE),
      );
      ...
      const { rows } = await pool.query<{ projection_version: string }>(
        'SELECT projection_version FROM chat_search.documents',
      );
      expect(Number(rows[0].projection_version)).toBe(first.projectionVersion);
    });

    it('still writes when only the source timestamp moved', async () => {
      await withTransaction(pool, (client) =>
        upsertDocument(client, lease.epoch, sourceOf(), DEFAULT_EMBEDDING_SPACE),
      );

      const touched = await withTransaction(pool, (client) =>
        upsertDocument(
          client,
          lease.epoch,
          sourceOf({ sourceUpdatedAt: new Date('2026-01-03T00:00:00Z') }),
          DEFAULT_EMBEDDING_SPACE,
        ),
      );

      expect(touched.applied).toBe(true);
      expect(await outboxCount()).toBe(2);
    });

    /** Reviving a tombstoned row is a real change even at an identical hash. */
    it('still writes when the stored row is a tombstone', async () => {
      await withTransaction(pool, (client) =>
        upsertDocument(client, lease.epoch, sourceOf(), DEFAULT_EMBEDDING_SPACE),
      );
      await withTransaction(pool, (client) => tombstoneDocument(client, lease.epoch, KEY));

      const revived = await withTransaction(pool, (client) =>
        upsertDocument(client, lease.epoch, sourceOf(), DEFAULT_EMBEDDING_SPACE),
      );
