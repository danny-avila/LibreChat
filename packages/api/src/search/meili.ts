...
/** Whether the caller asked for predicates this backend cannot push down. */
function hasListingFilters(filters: SearchFilters | undefined): boolean {
  if (!filters) {
    return false;
  }
  return filters.archived != null || (filters.tags?.length ?? 0) > 0 || filters.projectId != null;
}
...
export class MeiliChatSearch implements ChatSearch {
   ...
   *
   * Listing filters cannot be pushed down here, and that is reported rather than
   * hidden. The index declares `filterableAttributes: ['user']`, so `isArchived`,
   * `tags` and `chatProjectId` are not filterable at all; widening that list
   * forces Meilisearch to re-index on upgrade, which is precisely the cost a
   * deployment on this path was promised it would not pay. The caller's
   * post-hydration filter still produces a correct page — the candidates feeding
   * it were simply ranked without the filters, so a page can be shorter than it
   * would have been. Resolving the whole candidate window rather than one page's
   * worth is what keeps that shortfall from emptying a page outright.
   */
  async search(request: ChatSearchRequest): Promise<ChatSearchResult> {
    ...
    const degradations: SearchDegradation[] = hasListingFilters(request.filters)
      ? ['filters-unapplied']
      : [];
    if (!model) {
