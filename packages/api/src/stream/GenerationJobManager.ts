...
class GenerationJobManagerClass {
  ...
  private async processDurableReplacementReceipts(
    ...
    for (const receipt of receipts) {
      ...
      const hadRunningProvider =
        ...
        (receipt.providerAbortReady !== false || ownsInitializingLocalProvider);
      if (
  ...
  async abortJob(
    ...
    try {
      ...
      if (!abortSignalDelivered && !abortSignalPublished) {
        this.terminalProviderStopDeferred.add(terminalClaim);
      }
      ...
      // finally lands. The TTL lease covers live-process settlement while the user
      // document's durable abort fence lets deletion recovery rediscover the Stop
      // after this process exits. The retention is
      // handed to the DETERMINISTIC, heartbeat-held stop lease before the contender
      // lease releases, so
  ...
  private ownerLeaseHolds = new Map<string, OwnerLeaseHold>();

  /** Owner settlement can race a Redis outage even after every application write
   * has landed. Retry its exact-generation proof before dropping the owner marker,
   * otherwise a failed-handoff retainer has no way to distinguish natural
   * completion from a still-running predecessor. */
  private ownerSettlementRetryTimers = new Map<string, ReturnType<typeof setTimeout>>();

  /** Remote failed-handoff retainers share the deterministic store field with
   * the owner, but have an independent local lifetime: they stop on durable ACK
   * without clearing the owner's marker. */
  private predecessorRetentionHolds = new Map<string, PredecessorRetentionHold>();

  /** An undelivered Stop is terminal/invisible in the active set while its remote
   * provider may still be running. Keep that deterministic lease alive without
   * depending on a client retry to arrive before the store TTL. */
  private stopLeaseHolds = new Map<string, StopLeaseHold>();

  private ownerLeaseKey(streamId: string, createdAt: number): string {
    return `${streamId}:${createdAt}`;
  }

  private async retainStopLease(
    userId: string,
    streamId: string,
    tenantId: string | undefined,
    createdAt: number,
    ...
    let hold = this.stopLeaseHolds.get(key);
    if (hold == null) {
      const createdHold: StopLeaseHold = {
        ready: Promise.resolve(),
        releaseRequested: false,
      };
      hold = createdHold;
      this.stopLeaseHolds.set(key, createdHold);
      createdHold.ready = this.registerUserFinalization(
        userId,
        streamId,
        tenantId,
        createdAt,
        ...
        if (createdHold.releaseRequested) {
          return;
        }
        createdHold.heartbeat = this.createFinalizationHeartbeat(
          userId,
          streamId,
          tenantId,
          createdAt,
        ...
        const heartbeat = createdHold.heartbeat;
        this.scheduleFinalizationHeartbeat(heartbeat, async () => {
          // Another replica may deliver the Stop and record the exact-generation
          // proof. Retainers poll it so multiple origins cannot heartbeat the
          // deterministic stop field forever after delivery/settlement.
          const settled = await this.hasOwnerSettlementProof(streamId, createdAt);
          if (settled) {
            createdHold.releaseRequested = true;
            heartbeat.stopped = true;
            setImmediate(() => {
              void this.releaseStopLease(userId, streamId, tenantId, createdAt);
            });
            return;
          }
          await this.renewFinalizationHeartbeat(heartbeat);
        });
      });
    }
    try {
      ...
      if (!hold.releaseRequested && this.stopLeaseHolds.get(key) === hold) {
        this.stopLeaseHolds.delete(key);
      }
      throw error;
    }
    if (hold.releaseRequested) {
      throw new Error(`Stop lease for ${streamId}@${createdAt} was released during acquisition`);
    }
  }

  private async releaseStopLease(
    userId: string,
    streamId: string,
    tenantId: string | undefined,
    createdAt: number,
    ...
    const key = this.ownerLeaseKey(streamId, createdAt);
    const hold = this.stopLeaseHolds.get(key);
    if (hold != null) {
      hold.releaseRequested = true;
      await hold.ready.catch(() => undefined);
      if (hold.heartbeat != null) {
        await this.stopFinalizationHeartbeat(hold.heartbeat);
      } else {
        await this.jobStore
      ...
      if (this.stopLeaseHolds.get(key) === hold) {
        this.stopLeaseHolds.delete(key);
      }
      return;
    }
    await this.jobStore
      .clearUserFinalization?.(userId, streamId, tenantId, createdAt, USER_FINALIZATION_STOP_LEASE)
      .catch(() => undefined);
  }

  private createFinalizationHeartbeat(
    userId: string,
    streamId: string,
    tenantId: string | undefined,
    ...
    const heartbeat: FinalizationHeartbeat = {
      userId,
      streamId,
      tenantId,
      generationId,
      ...
      fallbackStore: this.userFinalizationFallback,
      fallbackLeaseKey: `fallback_${randomUUID()}`,
      fallbackActive: false,
      stopped: false,
      inFlight: Promise.resolve(),
      pendingPrimary: new Set(),
    };
    this.finalizationHeartbeats.add(heartbeat);
    return heartbeat;
  }

  /** Renews one primary lease. A rejected or five-second-stalled renewal promotes
   * the live holder into Mongo while the last confirmed Redis TTL still has almost
   * four minutes left. Once promoted, the fallback stays heartbeat-held until the
   * owner releases; primary recovery never opens a clear/recreate race. */
  private async renewFinalizationHeartbeat(heartbeat: FinalizationHeartbeat): Promise<void> {
    if (heartbeat.stopped) {
      return;
    }
    if (heartbeat.jobStore.registerUserFinalization == null) {
      throw new Error('Job store does not support finalization markers');
    }
    const primary = heartbeat.jobStore.registerUserFinalization(
      heartbeat.userId,
      heartbeat.streamId,
      heartbeat.tenantId,
    ...
    heartbeat.pendingPrimary.add(primary);
    void primary.then(
      () => heartbeat.pendingPrimary.delete(primary),
      () => heartbeat.pendingPrimary.delete(primary),
    );
    ...
    try {
      await withTimeout(
        primary,
        USER_FINALIZATION_RENEWAL_TIMEOUT_MS,
        `Finalization lease renewal timed out for ${heartbeat.streamId}`,
      );
    } catch (error) {
      primaryError = error;
    }
    ...
    if (primaryError != null) {
      const fallback = heartbeat.fallbackStore;
      if (fallback == null) {
        throw new Error(
          `Primary finalization lease failed without a configured fallback for ${heartbeat.streamId}`,
          { cause: primaryError },
        );
      }
      heartbeat.fallbackActive = true;
      await fallback.renew(
        heartbeat.userId,
        heartbeat.tenantId,
        heartbeat.fallbackLeaseKey,
        new Date(Date.now() + USER_FINALIZATION_FALLBACK_TTL_MS),
      );
      logger.warn(
        `[GenerationJobManager] Promoted finalization lease to Mongo fallback for ${heartbeat.streamId}`,
        primaryError,
      );
      return;
    }

    if (heartbeat.fallbackActive && heartbeat.fallbackStore != null) {
      await heartbeat.fallbackStore.renew(
        heartbeat.userId,
        heartbeat.tenantId,
        heartbeat.fallbackLeaseKey,
        new Date(Date.now() + USER_FINALIZATION_FALLBACK_TTL_MS),
  ...
  /** Recursive timeout rather than setInterval: a slow renewal can never overlap a
   * later one, and release serializes behind exactly one in-flight operation. */
  private scheduleFinalizationHeartbeat(
    heartbeat: FinalizationHeartbeat,
    tick: () => Promise<void> = () => this.renewFinalizationHeartbeat(heartbeat),
  ): void {
    if (heartbeat.stopped || heartbeat.timer != null) {
      return;
    }
    heartbeat.timer = setTimeout(() => {
      ...
      heartbeat.inFlight = heartbeat.inFlight
        .then(() => (heartbeat.stopped ? undefined : tick()))
        .catch((error) => {
          logger.error(
            `[GenerationJobManager] Finalization heartbeat failed for ${heartbeat.streamId}:`,
            error,
          );
        });
      void heartbeat.inFlight.then(() => {
        if (!heartbeat.stopped) {
          this.scheduleFinalizationHeartbeat(heartbeat, tick);
        }
      });
  ...
  private stopFinalizationHeartbeat(
    heartbeat: FinalizationHeartbeat,
    options: StopFinalizationHeartbeatOptions = {},
  ): Promise<void> {
    if (heartbeat.stopPromise != null) {
      return heartbeat.stopPromise;
    }
    const { clearPrimary = true, clearFallback = true, waitForPendingPrimary = true } = options;
    heartbeat.stopped = true;
    if (heartbeat.timer != null) {
      clearTimeout(heartbeat.timer);
      heartbeat.timer = undefined;
    }
    heartbeat.stopPromise = (async () => {
      await heartbeat.inFlight.catch(() => undefined);
      let pendingPrimarySettled = heartbeat.pendingPrimary.size === 0;
      if (!pendingPrimarySettled && waitForPendingPrimary) {
        pendingPrimarySettled = await withTimeout(
          Promise.allSettled([...heartbeat.pendingPrimary]).then(() => true),
          USER_FINALIZATION_RENEWAL_TIMEOUT_MS,
          `Finalization release timed out for ${heartbeat.streamId}`,
        ).catch(() => false);
      }
      // A late Redis renewal after HDEL would recreate the marker. When its reply
      // is still ambiguous, leave the primary key to its bounded TTL instead of
      // clearing underneath it. Persistence is already settled on explicit release.
      if (clearPrimary && pendingPrimarySettled) {
        await heartbeat.jobStore
          .clearUserFinalization?.(
            heartbeat.userId,
            heartbeat.streamId,
            heartbeat.tenantId,
      ...
      if (clearFallback && heartbeat.fallbackActive && heartbeat.fallbackStore != null) {
        await heartbeat.fallbackStore
          .clear(heartbeat.userId, heartbeat.tenantId, heartbeat.fallbackLeaseKey)
          .catch(() => undefined);
      }
      this.finalizationHeartbeats.delete(heartbeat);
    })();
    return heartbeat.stopPromise;
  }

  private async releaseAllFinalizationHeartbeats(): Promise<void> {
    const ownerHolds = [...this.ownerLeaseHolds.entries()];
    const predecessorHolds = [...this.predecessorRetentionHolds.entries()];
    const stopHolds = [...this.stopLeaseHolds.entries()];
    const heartbeats = [...this.finalizationHeartbeats];
    for (const [, hold] of ownerHolds) {
      hold.releaseRequested = true;
    }
    for (const [, hold] of predecessorHolds) {
      hold.stopped = true;
    }
    for (const [, hold] of stopHolds) {
      hold.releaseRequested = true;
    }
    for (const timer of this.ownerSettlementRetryTimers.values()) {
      clearTimeout(timer);
    }
    this.ownerSettlementRetryTimers.clear();
    // Manager teardown does not own the agents controllers' catch-side Mongo,
    // usage, and title promises. Stop local timers, but leave both primary and
    // fallback evidence to their crash TTLs; clearing here can let another replica
    // cascade a user while those detached writes are still landing.
    await Promise.allSettled(
      heartbeats.map((heartbeat) =>
        this.stopFinalizationHeartbeat(heartbeat, {
          clearPrimary: false,
          clearFallback: false,
          waitForPendingPrimary: false,
        }),
      ),
    );
    for (const [key, hold] of ownerHolds) {
      if (this.ownerLeaseHolds.get(key) === hold) {
        this.ownerLeaseHolds.delete(key);
      }
    }
    for (const [key, hold] of predecessorHolds) {
      if (this.predecessorRetentionHolds.get(key) === hold) {
        this.predecessorRetentionHolds.delete(key);
      }
    }
    for (const [key, hold] of stopHolds) {
      if (this.stopLeaseHolds.get(key) === hold) {
        this.stopLeaseHolds.delete(key);
      }
    }
  ...
  private async acquireOwnerLease(
    ...
    const key = this.ownerLeaseKey(streamId, createdAt);
    let hold = this.ownerLeaseHolds.get(key);
    if (hold == null) {
      const createdHold: OwnerLeaseHold = {
        ready: Promise.resolve(),
        releaseRequested: false,
      };
      hold = createdHold;
      // Install the state BEFORE registration starts. releaseOwnerLease can now
      // observe the in-flight acquire and clear only after its write settles.
      this.ownerLeaseHolds.set(key, createdHold);
      createdHold.ready = this.registerUserFinalization(
        userId,
        streamId,
        tenantId,
        ...
        this.scheduleFinalizationHeartbeat(createdHold.heartbeat);
      });
    }
    try {
      await hold.ready;
    } catch (error) {
      // A concurrent release owns the tombstone until its trailing clear lands.
      // Removing it here lets a third acquire register behind that clear.
      if (!hold.releaseRequested && this.ownerLeaseHolds.get(key) === hold) {
        this.ownerLeaseHolds.delete(key);
      }
      throw error;
    }
    if (hold.releaseRequested) {
      throw new Error(`Owner lease for ${streamId}@${createdAt} was released during acquisition`);
    }
  }
   ...
   * or failed for good.
   */
  private async recordOwnerSettlementProof(streamId: string, createdAt: number): Promise<boolean> {
    if (!this._isRedis || this.eventTransport.recordGenerationSettlement == null) {
      return true;
    }
    try {
      return await this.eventTransport.recordGenerationSettlement(streamId, createdAt);
    } catch (error) {
      logger.error(
        `[GenerationJobManager] Failed to record owner settlement proof for ${streamId}:`,
        error,
      );
      return false;
    }
  ...
  private async hasOwnerSettlementProof(streamId: string, createdAt: number): Promise<boolean> {
    try {
      if (
        this.eventTransport.hasAbortAcknowledgement != null &&
        (await this.eventTransport.hasAbortAcknowledgement(streamId, createdAt))
      ) {
        return true;
      }
      return (await this.eventTransport.hasGenerationSettlement?.(streamId, createdAt)) === true;
    } catch {
      return false;
    }
  }

  private scheduleOwnerSettlementRetry(
    userId: string,
    streamId: string,
    tenantId: string | undefined,
    createdAt: number,
    ...
    const key = this.ownerLeaseKey(streamId, createdAt);
    if (this.ownerSettlementRetryTimers.has(key) || this.shuttingDown) {
      return;
    }
    const timer = setTimeout(() => {
      if (this.ownerSettlementRetryTimers.get(key) === timer) {
        this.ownerSettlementRetryTimers.delete(key);
      }
      void this.releaseOwnerLease(userId, streamId, tenantId, createdAt).catch((error) => {
        logger.error(
          `[GenerationJobManager] Owner settlement retry failed for ${streamId}:`,
          error,
        );
      });
    }, USER_FINALIZATION_RENEWAL_MS);
    timer.unref?.();
    this.ownerSettlementRetryTimers.set(key, timer);
  }
  ...
  async releaseOwnerLease(
    ...
    // Generalize the existing abort acknowledgement into exact-generation owner
    // settlement proof. A failed-handoff retainer can then stop when its predecessor
    // completes naturally, not only when it receives the abort signal.
    const settlementProofRecorded = await this.recordOwnerSettlementProof(streamId, createdAt);
    let observedJob: SerializableJobData | null | undefined;
    try {
      observedJob = await this.jobStore.getJob(streamId);
    } catch {
      observedJob = undefined;
    }
    let durableAbortFenceCleared = true;
    // Avoid a Mongo User mutation/cache invalidation on every normal completion.
    // Only an exact aborted generation can own durable Stop-delivery evidence.
    if (
      (hold != null ||
        (observedJob?.createdAt === createdAt && observedJob.status === 'aborted')) &&
      this.userFinalizationFallback?.clearAbortDelivery != null
    ) {
      durableAbortFenceCleared = await this.userFinalizationFallback
        .clearAbortDelivery(userId, tenantId, streamId)
        .then(() => true)
        .catch((error) => {
          logger.error(
            `[GenerationJobManager] Failed to clear durable abort delivery fence for ${streamId}:`,
            error,
          );
          return false;
        });
    }
    if (!settlementProofRecorded || !durableAbortFenceCleared) {
      // Keep (or create) a heartbeat while exact settlement proof is retried. The
      // owner writes are finished, so this is conservative availability fencing.
      if (hold == null) {
        await this.acquireOwnerLease(userId, streamId, tenantId, createdAt).catch((error) => {
          logger.error(
            `[GenerationJobManager] Could not retain owner marker for settlement retry ${streamId}:`,
            error,
          );
        });
        hold = this.ownerLeaseHolds.get(key);
      }
      this.scheduleOwnerSettlementRetry(userId, streamId, tenantId, createdAt);
      return;
    }

    const retryTimer = this.ownerSettlementRetryTimers.get(key);
    if (retryTimer != null) {
    ...
    if (hold != null) {
      hold.releaseRequested = true;
      // Registration may already be in flight. Clear only after it settles so
      // the acquire cannot recreate the marker behind this release.
      await hold.ready.catch(() => undefined);
      if (hold.heartbeat != null) {
        await this.stopFinalizationHeartbeat(hold.heartbeat);
      } else {
    ...
    // Cross-replica aborts settle in the controller catch rather than through
    // finishTerminalJob on this manager. Once those writes release the owner
    // lease, retire the exact terminal ownership so cleanup can reap its runtime.
    const runtime = this.runtimeState.get(streamId);
    if (runtime?.createdAt === createdAt && runtime.abortController.signal.aborted) {
      try {
        const job = observedJob === undefined ? await this.jobStore.getJob(streamId) : observedJob;
        if (
          job?.createdAt !== createdAt ||
          (job.status !== 'running' && job.status !== 'requires_action')
        ) {
        ...
        logger.warn(
          `[GenerationJobManager] Could not verify terminal owner release for ${streamId}:`,
          error,
        );
      }
    }
   ...
   * remote owner settles, so the renewal predicate accepts either durable abort
   * acknowledgement or exact-generation owner-settlement proof. Once one exists,
   * this retainer stops renewing WITHOUT clearing the shared field.
   ...
   */
  private async retainPredecessorOwnerLease(
    userId: string,
    streamId: string,
    tenantId: string | undefined,
    createdAt: number,
    ...
    const key = this.ownerLeaseKey(streamId, createdAt);
    const ownerHold = this.ownerLeaseHolds.get(key);
    if (ownerHold != null) {
      await ownerHold.ready;
      if (ownerHold.releaseRequested) {
        throw new Error(`Owner lease for ${streamId}@${createdAt} is being released`);
      }
      return;
    }
    const existing = this.predecessorRetentionHolds.get(key);
    if (existing != null) {
      await existing.ready;
      return;
    }
    const hold: PredecessorRetentionHold = {
      ready: Promise.resolve(),
      stopped: false,
    };
    this.predecessorRetentionHolds.set(key, hold);
    hold.ready = (async () => {
      if (await this.hasOwnerSettlementProof(streamId, createdAt)) {
        hold.stopped = true;
        return;
      }
      // Mandatory initial proof. A replacement must remain active/discoverable
      // when this write fails; a timer that might succeed later cannot cover the
      // gap after its caller releases the admission lease.
      ...
      if (!hold.stopped) {
        hold.heartbeat = this.createFinalizationHeartbeat(
          userId,
          streamId,
          tenantId,
          createdAt,
    ...
    const heartbeat = hold.heartbeat;
    if (heartbeat == null) {
      return;
    }
    this.scheduleFinalizationHeartbeat(heartbeat, async () => {
   ...
   * (callers fail closed); a failed or stalled renewal promotes to a Mongo-backed
   * fallback before the primary TTL can expire.
   */
  async holdUserFinalization(
    ...
    this.scheduleFinalizationHeartbeat(heartbeat);
    let releasePromise: Promise<void> | undefined;
    const release = (): Promise<void> => {
      releasePromise ??= this.stopFinalizationHeartbeat(heartbeat);
