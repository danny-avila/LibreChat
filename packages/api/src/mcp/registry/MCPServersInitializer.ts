...
export class MCPServersInitializer {
   ...
   *
   * Design rationale:
   * - Uses in-memory `initialized` flag (not cache-based) to prevent cross-deployment contamination.
   *   Each deployment must initialize its own instance to ensure configs are loaded correctly.
  ...
  public static async initialize(rawConfigs: t.MCPServers): Promise<void> {
    ...
    if (await isLeader()) {
      ...
      if (!(await statusCache.isInitialized())) await this.initialize(rawConfigs);
    }
  ...
  /** Resets the static initialized flag (for testing purposes) */
