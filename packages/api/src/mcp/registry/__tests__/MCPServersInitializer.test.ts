...
describe('MCPServersInitializer', () => {
  ...
  beforeEach(async () => {
    ...
    // Reset caches and initializer before each test
    MCPServersInitializer.reset();
  ...
  describe('initialize()', () => {
    it('should reset registry and status cache before initialization', async () => {
      ...
      expect(await registry.sharedAppServers.get('old_app_server')).toBeDefined();
      expect(await registry.sharedUserServers.get('old_user_server')).toBeDefined();
    ...
    it('should skip initialization if already initialized (static flag)', async () => {
