...
import type { UploadImageParams, ImageUploadResult, ProcessAvatarParams } from '../types';
...
export interface S3ImageServiceDeps {
  ...
  updateFile: (params: { file_id: string }) => Promise<void>;
}
...
export class S3ImageService {
  ...
  async prepareImageURL(file: { file_id: string; filepath: string }): Promise<[void, string]> {
    try {
