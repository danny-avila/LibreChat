import { logger, webSearchKeys } from '@brainiac/data-schemas';
import { Constants, extractVariableName } from 'brainiac-data-provider';
...
const deprecatedVariables = [
  {
    key: 'CHECK_BALANCE',
    description:
      'Please use the `balance` field in the `brainiac.yaml` config file instead.\nMore info: https://brainiac.ai/docs/configuration/brainiac_yaml/object_structure/balance#overview',
  },
  {
    key: 'START_BALANCE',
    description:
...
function checkPasswordReset() {
  ...
  if (!emailEnabled && passwordResetAllowed) {
    logger.warn(
      ...
      https://www.brainiac.ai/docs/configuration/authentication/email
...
export function checkVariables() {
  ...
  if (hasDefaultSecrets) {
    ...
    https://www.brainiac.ai/toolkit/creds_generator
...
Latest version: ${Constants.CONFIG_VERSION}
      ...
      https://www.brainiac.ai/changelog\n\n`,
    );
  }
}
    ...
    if (typeof value === 'string') {
      ...
      if (varName) {
        ...
        logger.warn(
          ...
          This is incorrect! You should use environment variable references in your brainiac.yaml file, such as:
