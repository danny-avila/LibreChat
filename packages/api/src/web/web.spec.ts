...
describe('web.ts', () => {
  ...
  describe('crw (fastCRW) provider', () => {
    ...
    it('should authenticate crw as both search provider and scraper with a shared key', async () => {
      ...
      expect(result.authResult.crwSearchOptions).toEqual(webSearchConfig.crwSearchOptions);
      expect(result.authResult.crwScraperOptions).toEqual(webSearchConfig.crwScraperOptions);
      expect(result.authResult.scraperTimeout).toBe(20000);
    });

    it('should fall back to default timeout when crwScraperOptions.timeout is absent', async () => {
      const webSearchConfig: TCustomConfig['webSearch'] = {
        serperApiKey: '${SERPER_API_KEY}',
        crwApiKey: '${CRW_API_KEY}',
        jinaApiKey: '${JINA_API_KEY}',
      ...
      expect(result.authenticated).toBe(true);
      expect(result.authResult.scraperTimeout).toBe(7500);
      expect(result.authResult.crwScraperOptions).toEqual({ formats: ['markdown'] });
    });
