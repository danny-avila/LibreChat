...
import { assertCanonicalGeneratedBamlSourcePaths } from './bamlGeneratedPaths.mjs';
import {
  FINGERPRINT_FILE,
  GENERATED_DIR,
  SOURCE_DIR,
...
if (!exists(FINGERPRINT_FILE)) {
  fail(`Missing ${FINGERPRINT_FILE}\nRun: npm --prefix packages/api run generate:baml`);
}

try {
  assertCanonicalGeneratedBamlSourcePaths();
} catch (error) {
  fail(
    `BAML generated bytecode is not canonical:\n${error instanceof Error ? error.message : error}\n` +
      'Run: npm --prefix packages/api run generate:baml',
  );
}
