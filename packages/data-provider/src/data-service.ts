...
/**
 * List MCP servers accessible to the user
 */
export const getMcpServers = async (params?: {
  limit?: number;
  after?: string;
  search?: string;
}): Promise<mcp.McpServerListResponse> => {
  return request.get(endpoints.mcpServers(params));
};

/**
 * Get a single MCP server by ID
 */
export const getMcpServer = async (mcp_id: string): Promise<mcp.McpServer> => {
 ...
 */
export const createMcpServer = async (data: mcp.McpServerCreateParams): Promise<mcp.McpServer> => {
  return request.post(endpoints.mcpServers(), data);
};
