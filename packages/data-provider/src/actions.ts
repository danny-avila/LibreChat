 ...
 * Used for operation hashing and deduplication.
 *
 * Note: This was previously SHA-1 but changed to SHA-256 for FIPS compliance.
 * SHA-1 is not approved for new applications in FIPS 140-2/140-3.
 *
 * @deprecated Use operationHash instead for clarity
 */
export function sha1(input: string) {
  // Using SHA-256 despite the function name for FIPS compliance
  // Function name kept for backward compatibility with existing code
  return crypto.createHash('sha256').update(input).digest('hex');
}

/**
 * Generates a SHA-256 hash of the input string for operation identification.
 * FIPS 140-2/140-3 compliant.
 */
export function operationHash(input: string) {
  return crypto.createHash('sha256').update(input).digest('hex');
}
