...
## 10. 2026-01-07 架构优化与关键修复

### 10.1 图片路径架构简化 ⭐

**问题背景**：
图片路径出现双重嵌套，如 `/images/userId/timestamp-/images/userId/timestamp-plot-0.png`

**根本原因**：
1. LLM 在多轮对话中会引用之前生成的图片路径
2. 路径替换逻辑会匹配到已经正确的路径中的子串（如 `plot-0.png`）
3. 导致对正确路径再次进行替换，造成嵌套

**解决方案** - 架构级简化：
```javascript
// 在 tools.js 中直接提供正确路径
observation.image_paths = persistedFiles.map(f => f.filepath);
observation.images_markdown = persistedFiles.map((f, i) => 
  `![Plot ${i}](${f.filepath})`
).join('\n');

// 在 index.js 中完全移除路径替换
const processedText = finalContent; // 直接使用，不做任何替换
```

**影响范围**：
- `api/server/services/Agents/e2bAgent/tools.js` - 增强 observation 格式
- `api/server/services/Agents/e2bAgent/index.js` - 移除 `replaceImagePaths()` 逻辑
- `api/server/services/Agents/e2bAgent/prompts.js` - 更新 System Prompt
...
- 成功时：`{ success: true, stdout, stderr, has_plots, plot_count, image_paths, ... }`
...
### 10.3 工具精简 - 移除 download_file

**发现**：`download_file` 工具与 `execute_code` 的自动图片持久化功能完全冗余

**修改内容**：
1. 从 `tools.js` 中移除 `download_file` 函数
2. 从 `prompts.js` 中移除工具定义
3. 更新 System Prompt，强调自动图片持久化
4. 修复 `initialize.js` 中的 E2B downloadFile API 错误
...
为诊断复杂问题（如高级统计分析失败），添加了全面的 debug 日志：
