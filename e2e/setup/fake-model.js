...
function mcpLegacyResponses(label, toolNames) {
  const toolName = Array.from(toolNames).find((name) => name.startsWith('show_legacy'));
  if (!toolName) {
    return { responses: ['E2E legacy MCP-UI unavailable: show_legacy was not advertised.'] };
  }
  return {
    responses: [''],
    toolCalls: [
      {
        id: `call_e2e_mcp_legacy_${label}`,
        name: toolName,
        args: { label },
        type: 'tool_call',
      },
    ...
    resolveOnStream: (streamMessages) => {
      ...
      return resourceId ? { responses: [`Legacy MCP-UI: \\ui{${resourceId}}`] } : null;
    },
  };
}
