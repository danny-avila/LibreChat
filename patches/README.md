...
2. Pack the SDK with `npm pack`. Unpack that artifact and a fresh `npm pack @librechat/agents@3.9.0` artifact into separate package trees. Generate the patch from their production package contents, retaining matching source, CJS, ESM and declarations; do not copy test fixtures into npm `files`.
